...
public class ScheduleCompositeTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        ...
        game = new Engine();
        testScheduleComposite = new DummyScheduleComposite(game,
                "DummyScheduleComposite", null);
        for (int i = 0; i < 10; i++) {
            dummySchedules.add(new DummySchedule(game, "DummySchedule" + i,
                    testScheduleComposite));
        }
        dummyScheduleItem = new DummyScheduleItem(game, "DummyScheduleItem",
                testScheduleComposite);
    }
    ...
    public void testConstructor() {
        assertEquals("No schedules expected", 0, testScheduleComposite
                .getChildCount());
    }
    ...
    private void addDummySchedules(int nods) {
        for (int i = 0; i < nods; i++) {
            testScheduleComposite
                    .addAfterLastChild((DummySchedule) dummySchedules.get(i));
        }
    }

    private void assertExecuteRunUndoRedo(int numberOfDummySchedules) {
        addDummySchedules(numberOfDummySchedules);
        assertEquals(numberOfDummySchedules, testScheduleComposite
                .getChildCount());

        assertExecute(numberOfDummySchedules, IControlFlowModel.Running,
                IControlFlowModel.Running, -1, numberOfDummySchedules);
        assertExecute(numberOfDummySchedules, IControlFlowModel.Undoing,
                IControlFlowModel.Undoing, numberOfDummySchedules, -1);
        assertExecute(numberOfDummySchedules, IControlFlowModel.Redoing,
                IControlFlowModel.Redoing, -1, numberOfDummySchedules);
    }

    private void assertExecute(int numberOfDummySchedules, int gameState,
            int expectedGameState, int initialIndex, int expectedIndex) {
    ...
    public void testExecuteWithScheduleItem() {
        ...
        assertExecuteWithScheduleItem(IControlFlowModel.Running,
                IControlFlowModel.Running, -1, true, false);
        assertExecuteWithScheduleItem(IControlFlowModel.Undoing, -1,
                IControlFlowModel.Undoing, false, true);
        assertExecuteWithScheduleItem(IControlFlowModel.Redoing,
                IControlFlowModel.Redoing, -1, true, false);
    }

    private void assertExecuteWithScheduleItem(int gameState,
            int expectedExecuteGameState, int expectedUndoGameState,
            boolean expectedIsExecuteVisited, boolean expectedIsUndoVisited) {
        ...
        assertEquals(expectedExecuteGameState, dummyScheduleItem
                .getExecuteGameState());
        assertEquals(expectedUndoGameState, dummyScheduleItem
                .getUndoGameState());
        assertEquals(expectedIsExecuteVisited, dummyScheduleItem
                .isExecuteVisited());
